...
export function SuperPartnerHome() {
  ...
  return (
    <MobileShell title="D3 多签系统">
      ...
      {error && (
        <div className="brand-card rounded-2xl p-4 text-[13px] text-red-500 font-medium">{error}</div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        {!loading && wallets.length === 0 && !error && (
          <div className="col-span-2 brand-card rounded-2xl p-4 text-[13px] text-[#8A2B57]/60 text-center">
            暂无钱包数据
          </div>
        )}
      </div>
      ...
      <a
        href="#/transfer"
        className="tap flex items-center justify-between brand-gradient text-white rounded-2xl px-4 py-3.5"
      >
