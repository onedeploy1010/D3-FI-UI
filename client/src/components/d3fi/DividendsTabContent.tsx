...
const EMISSION_SEGMENTS = [
  { pct: 65, color: '#E0568F', icon: Vote, labelZh: 'Gauge 投票者', labelEn: 'Gauge voters' },
  { pct: 15, color: '#B23A6E', icon: Shield, labelZh: 'veD3 底薪', labelEn: 'veD3 base', subZh: '锁仓 ≥180 天', subEn: 'lock ≥180d' },
  { pct: 10, color: '#8A2B57', icon: Crown, labelZh: 'DT Top5', labelEn: 'DT Top5', subZh: '持仓排名前五', subEn: 'top 5 holders' },
...
function EmissionSplitVisual({ lang, isDark }: { lang: Lang; isDark: boolean }) {
  ...
  return (
    <div>
      <div className={`h-2.5 rounded-full overflow-hidden flex ${isDark ? 'ring-1 ring-white/[0.06]' : 'ring-1 ring-[#8A2B57]/[0.06]'}`}>
      ...
      <div className="mt-4 space-y-1">
        {EMISSION_SEGMENTS.map((seg) => {
          ...
          return (
            <div
              key={seg.labelEn}
              className={`flex items-center gap-3 py-2.5 px-1 border-b last:border-0 ${isDark ? 'border-white/[0.04]' : 'border-[#8A2B57]/[0.05]'}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-xs font-semibold ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? seg.labelZh : seg.labelEn}</div>
                <div className={`text-[10px] mt-0.5 ${isDark ? 'text-white/30' : 'text-[#160510]/35'}`}>{t ? seg.subZh : seg.subEn}</div>
              </div>
              <div className="text-sm font-bold font-stat shrink-0" style={{ color: seg.color }}>{seg.pct}%</div>
            </div>
          );
        })}
      </div>
    ...
    <>
      ...
      <div className="flex gap-2 mb-3">
        {(['7d', '30d', 'all'] as const).map((r) => (
          <button
            ...
            type="button"
            onClick={() => onRange(r)}
            className={cn('flex-1 ios-glass-inset py-2 text-[10px] font-semibold ios-glass-pressable', range === r && 'ring-1 ring-[#E0568F]/20')}
          >
            {r !== 'all' ? (
              <>
      ...
      <button type="button" onClick={onSort} className="w-full ios-glass-inset p-3 text-left ios-glass-pressable">
        <div className="flex items-center justify-between">
          <span className={`text-[10px] ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? '排序' : 'Sort'}</span>
          <span className={`text-xs font-semibold ${isDark ? 'text-white' : 'text-[#160510]'}`}>
            <ArrowUpDown size={12} className="inline mr-1 text-[#E0568F]" />
            {sortLabels[0]}
          </span>
        </div>
      </button>
      ...
      <div className="space-y-4">
        <div className={glassCardClass('default', 'p-4 sm:p-5')}>
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              <div className={`text-[10px] font-semibold tracking-wider uppercase ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>
                Epoch {epoch}
              </div>
              <div className={`text-sm font-bold ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? '本期收益明细' : 'Epoch Reward Breakdown'}</div>
            </div>
            ...
            {categoryChips.map((chip) => (
              <button
                ...
                className={cn(
                  ...
                  breakdownFilter === chip.id ? (isDark ? 'text-white' : 'text-[#8A2B57]') : (isDark ? 'text-white/40' : 'text-[#160510]/40'),
                )}
              >
          ...
          <div className="mt-4 space-y-2">
              ...
              return (
                <div key={item.id} className="ios-glass-inset p-3 flex items-center gap-3">
                  <div className="ios-glass-inset w-9 h-9 flex items-center justify-center shrink-0">
                    <Icon size={14} className="text-[#E0568F]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`text-xs font-medium truncate ${isDark ? 'text-white' : 'text-[#160510]'}`}>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className={`text-[9px] mt-0.5 ${item.status === 'claimable' ? 'text-emerald-500/70' : isDark ? 'text-white/25' : 'text-[#160510]/25'}`}>
            ...
            {filteredBreakdown.length === 0 && (
              <div className={`text-center py-8 text-xs ${isDark ? 'text-white/30' : 'text-[#160510]/35'}`}>
        ...
        <div className={glassCardClass('default', 'p-4 sm:p-5')}>
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              ...
              <div className={`text-sm font-bold ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? '领取历史' : 'Claim History'}</div>
            </div>
            ...
            {filteredHistory.map((item) => (
              <div key={item.id} className="ios-glass-inset p-3">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className={`text-xs font-medium ${isDark ? 'text-white' : 'text-[#160510]'}`}>{item.date}</div>
                    <div className={`text-[10px] mt-0.5 ${isDark ? 'text-white/30' : 'text-[#160510]/35'}`}>
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-[9px] ${isDark ? 'text-white/25' : 'text-[#160510]/25'}`}>USDT</div>
                  </div>
                </div>
    ...
    <div className="space-y-5">
      <div className={glassCardClass('green', 'p-5 relative overflow-hidden')}>
        ...
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className={`text-xs ${isDark ? 'text-white/40' : 'text-[#160510]/40'}`}>{t ? '待领取收益' : 'Claimable Rewards'}</div>
          <GlassIconButton onClick={() => openRules('claim')} aria-label="Open claim rules">
            <HelpCircle size={16} className={isDark ? 'text-white/40' : 'text-[#160510]/40'} />
          </GlassIconButton>
        </div>
        <div className="text-3xl font-bold text-emerald-500 font-stat">${claimable.usdt.toFixed(2)}</div>
        <div className={`text-[10px] mt-1 mb-4 ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>
          {t ? '治理分红 · USDT 直接领取到钱包' : 'Governance dividends · USDT claimed directly to wallet'}
        </div>

        <div className={`rounded-xl p-4 mb-2 ${isDark ? 'bg-white/[0.03]' : 'bg-[#8A2B57]/[0.03]'}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-[10px] ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? '贿赂 + LP + 权重分红' : 'Bribes + LP + weight share'}</span>
              </div>
              <div className={`text-2xl font-bold font-stat ${isDark ? 'text-white' : 'text-[#160510]'}`}>
        ...
        <button
          ...
          className={`w-full text-[10px] font-semibold ios-glass-pressable py-2 ${isDark ? 'text-[#E0568F]/80' : 'text-[#8A2B57]/75'}`}
        >
      ...
      <div className={glassCardClass('default', 'p-5')}>
        ...
        <div className="ios-glass-inset p-4 mb-3">
          <div className="flex items-center justify-between mb-2">
            <span className={`text-[10px] ${isDark ? 'text-white/40' : 'text-[#160510]/40'}`}>{t ? '当前全网质押率' : 'Network Staking Rate'}</span>
            <span className={`text-sm font-bold ${isDark ? 'text-[#E0568F]' : 'text-[#8A2B57]'}`}>{stakingRate}%</span>
          </div>
          <div className={`h-2 rounded-full overflow-hidden ${isDark ? 'bg-white/[0.06]' : 'bg-[#8A2B57]/[0.06]'}`}>
            <div className="h-full rounded-full bg-gradient-to-r from-[#8A2B57] to-[#E0568F]" style={{ width: `${stakingRate}%` }} />
          </div>
          <div className={`text-[10px] mt-2 ${isDark ? 'text-white/30' : 'text-[#160510]/35'}`}>
            {t ? '质押池' : 'Pool'}: 5.2M / 6.3M D3 · {poolPhase === 'growth' ? (t ? '增长期' : 'Growth') : (t ? '可持续期' : 'Sustain')}
          </div>
        </div>
        <div className="space-y-2 text-[11px]">
          <div className={`p-3 rounded-xl ${poolPhase === 'growth' ? 'ring-1 ring-[#E0568F]/20' : ''} ${isDark ? 'bg-white/[0.02]' : 'bg-[#8A2B57]/[0.02]'}`}>
            <div className={`font-semibold mb-1 ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? '增长期（池 > 6.3M）' : 'Growth (pool > 6.3M)'}</div>
            <p className={`text-[10px] ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>
              {t ? '质押率越高 → 日化收益越高' : 'Higher stake rate → higher daily yield'}
            </p>
          </div>
          <div className={`p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-[#8A2B57]/[0.02]'}`}>
            <div className={`font-semibold mb-1 ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? '可持续期（池 ≤ 6.3M）' : 'Sustain (pool ≤ 6.3M)'}</div>
            <p className={`text-[10px] ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>
      ...
      <div className={glassCardClass('highlight', 'p-5 relative overflow-hidden')}>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <div className={`text-[10px] font-semibold tracking-wider uppercase ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>Epoch {epoch}</div>
            <div className={`text-xs font-bold mt-0.5 ${isDark ? 'text-white' : 'text-[#160510]'}`}>{t ? '当期贿赂池' : 'Current Bribe Pool'}</div>
          </div>
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            <div className="text-3xl font-bold font-stat" style={{ color: isDark ? '#E0568F' : '#8A2B57' }}>{bribePoolTotal}</div>
            <div className={`text-[10px] mt-1 ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? 'USDT 贿赂总额' : 'Total USDT bribes'}</div>
          </div>
          <div className="text-right">
            <div className={`text-lg font-bold font-stat ${isDark ? 'text-white' : 'text-[#160510]'}`}>{bribePoolDistributableD3}</div>
            <div className={`text-[10px] mt-0.5 ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? '可分配 D3' : 'Distributable D3'}</div>
          </div>
        </div>
      </div>
      ...
      <div className={glassCardClass('default', 'p-5')}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="ios-glass-inset p-4 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-16 h-16 rounded-full blur-2xl opacity-30" style={{ background: '#8A2B57' }} />
            <div className="relative">
              <div className="flex items-center gap-2 mb-3">
                <div className="ios-glass-inset w-8 h-8 flex items-center justify-center">
                  <Coins size={14} className="text-[#8A2B57]" />
                </div>
                <span className={`text-xs font-semibold ${isDark ? 'text-white' : 'text-[#160510]'}`}>DT {t ? '权重分红' : 'Weight Share'}</span>
              </div>
              <div className="text-2xl font-bold font-stat text-emerald-500">${estimatedDtUsdt.toLocaleString()}</div>
              <div className={`text-[10px] mt-1 mb-3 ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? '本期预估 USDT' : 'Epoch USDT estimate'}</div>
              <div className="flex items-center justify-between text-[10px] mb-1.5">
                <span className={isDark ? 'text-white/40' : 'text-[#160510]/40'}>{t ? '我的权重分红率' : 'Your weight rate'}</span>
                <span className={`font-bold ${isDark ? 'text-[#E0568F]' : 'text-[#8A2B57]'}`}>{myDtRate.toFixed(2)}%</span>
              </div>
              <div className={`h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-white/[0.06]' : 'bg-[#8A2B57]/[0.06]'}`}>
                <div className="h-full rounded-full bg-gradient-to-r from-[#8A2B57] to-[#E0568F]" style={{ width: `${Math.min(100, myDtRate * 10)}%` }} />
              </div>
              <div className={`flex justify-between mt-2 text-[10px] ${isDark ? 'text-white/30' : 'text-[#160510]/30'}`}>
          ...
          <div className="ios-glass-inset p-4 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-16 h-16 rounded-full blur-2xl opacity-30" style={{ background: '#E0568F' }} />
            <div className="relative">
              <div className="flex items-center gap-2 mb-3">
                <div className="ios-glass-inset w-8 h-8 flex items-center justify-center">
                  <Vote size={14} className="text-[#E0568F]" />
                </div>
                <span className={`text-xs font-semibold ${isDark ? 'text-white' : 'text-[#160510]'}`}>veD3 {t ? '投票收益' : 'Voting Rewards'}</span>
              </div>
              <div className="text-2xl font-bold font-stat text-emerald-500">${estimatedVoteUsd.toFixed(2)}</div>
              <div className={`text-[10px] mt-1 mb-3 ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>{t ? '贿赂 + 排放 + LP 费' : 'Bribe + emission + LP fees'}</div>
              <div className="flex items-center justify-between text-[10px] mb-1.5">
                <span className={isDark ? 'text-white/40' : 'text-[#160510]/40'}>{t ? '已投 veD3' : 'Allocated veD3'}</span>
                <span className={`font-bold ${isDark ? 'text-[#E0568F]' : 'text-[#8A2B57]'}`}>{myVeD3.toLocaleString()}</span>
              </div>
              <div className={`h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-white/[0.06]' : 'bg-[#8A2B57]/[0.06]'}`}>
                <div className="h-full rounded-full bg-[#E0568F]" style={{ width: '33%' }} />
              </div>
              <div className={`flex justify-between mt-2 text-[10px] ${isDark ? 'text-white/30' : 'text-[#160510]/30'}`}>
      ...
      <div className={glassCardClass('default', 'p-5')}>
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            <div className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-white/50' : 'text-[#160510]/40'}`}>{t ? 'D3 排放分配' : 'D3 Emission Split'}</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <div className={`text-[10px] font-semibold uppercase tracking-wider ${isDark ? 'text-white/35' : 'text-[#160510]/35'}`}>
            {t ? '收益明细' : 'Breakdown'}
          </div>
          <div className={`text-sm font-bold mt-1 ${isDark ? 'text-white' : 'text-[#160510]'}`}>{breakdownItems.length} {t ? '条' : 'items'}</div>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <div className={`text-sm font-bold mt-1 ${isDark ? 'text-white' : 'text-[#160510]'}`}>{historyItems.length} {t ? '笔' : 'claims'}</div>
        </button>
      </div>
