  ...
  const sections: { id: RuleTopic; titleZh: string; titleEn: string; bulletsZh: string[]; bulletsEn: string[] }[] = [
    ...
    },
    {
      ...
      bulletsZh: [
        ...
        '推荐奖励（USD3）在「我的 → 推荐」或「资产 → USD3」入账，用于质押投资，不在此领取。',
        '每次领取单独消耗 gas。',
      ],
      bulletsEn: [
        ...
        'Referral rewards (USD3) credit on Me → Refer or Assets → USD3 for staking — not claimed here.',
    ...
    },
    {
      ...
      bulletsZh: [
        ...
        '推荐奖励 = 下级入金的 30%，全部以 USD3 入账，用于质押投资，不可提现。',
        'USD3 30% 中：15% 自留可质押；15% 为可转让额度，仅可转给直推下线用于质押。',
      ...
      bulletsEn: [
        ...
        'Referral reward = 30% of downline entry, all credited as USD3 for staking/investment — not withdrawable.',
        'Within the 30% USD3: 15% self for staking; 15% transferable quota to direct downline only.',
    ...
    },
    {
      id: 'dusd',
      titleZh: 'USD3 与可转让额度',
      titleEn: 'USD3 & Transferable Quota',
      bulletsZh: [
        'USD3 为协议内质押专用资产（USDT 1:1 铸造），不可在 DEX 交易，也不可提现。',
        '用于 LP/销毁债券入场质押，以及接收推荐奖励（入金 30%，全部为 USD3）。',
        '推荐奖励入账后用于投资质押；其中 15% 为可转让额度，仅可转给直推下线。',
        '治理分红（贿赂、LP、权重分红等）为 USDT，在收益页直接领取到钱包。',
      ],
      bulletsEn: [
        'USD3 is the in-protocol staking asset (minted 1:1 with USDT); not DEX-tradable or withdrawable.',
        'Used for LP/burn bond staking and referral rewards (30% of entry, all USD3).',
        'Referral USD3 credits for staking/investment; 15% is transferable quota to direct downline only.',
        ...
        >
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 mb-4 relative z-10">
              <div>
                <div className={`text-[10px] font-semibold tracking-wider uppercase ${isDark ? 'text-[#E0568F]/55' : 'text-[#8A2B57]/45'}`}>
                  {t ? '规则' : 'Rules'}
                </div>
                <div className={`text-lg font-bold font-heading ${isDark ? 'text-white' : 'text-[#160510]'}`}>
            ...
            <div className="space-y-4 relative z-10">
              {ordered.map((sec) => (
                <div key={sec.id} className="ios-glass-inset p-4">
                  <div className={`text-sm font-semibold mb-2 ${isDark ? 'text-white' : 'text-[#160510]'}`}>
                    {t ? sec.titleZh : sec.titleEn}
                  </div>
                  <ul className={`space-y-1 text-[11px] leading-relaxed ${isDark ? 'text-white/45' : 'text-[#160510]/50'}`}>
