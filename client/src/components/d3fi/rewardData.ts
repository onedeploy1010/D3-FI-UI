/** Reward taxonomy aligned with whitepaper + market PPT + UD3 product layer */

/** Referral reward on downline entry: 30% total, 100% UD3 (for staking/investment, not withdrawal) */
...
/** Epoch #42 — downline entry × 30%, all UD3 */
export const referralEpoch = {
  epoch: '#42',
  /** Total accrued this epoch (e.g. $2,000 downline entry × 30%) */
  total: 600,
  /** Already credited to UD3 balance / transferable quota */
  claimed: 360,
};
...
/** UD3 account snapshot — numbers must stay internally consistent */
...
export const rewardStreams: RewardStream[] = [
  ...
  },
  {
    ...
    assetEn: '100% UD3',
    whereZh: '推荐页 + 资产 → UD3',
    whereEn: 'Refer tab + Assets → UD3',
    descZh: '下级入金的 30% 全部以 UD3 入账，用于质押投资，不可提现。其中 15% 自留 + 15% 可转让直推下线。',
    descEn: '30% of downline entry paid entirely in UD3 for staking/investment — not withdrawable. 15% self + 15% transferable to direct downline.',
  },
  {
    id: 'dusd',
    titleZh: 'UD3 推荐份额',
    titleEn: 'UD3 referral share',
    assetZh: 'UD3（入金 30%）',
    assetEn: 'UD3 (30% of entry)',
    whereZh: '资产 → UD3',
    whereEn: 'Assets → UD3',
    descZh: '推荐奖励全部记入 UD3 余额；可转让额度仅来自其中 15%，且只能转给直推下线用于质押。',
    descEn: 'All referral rewards credit as UD3; transferable quota is the 15% slice only, to direct downline for staking.',
  },
];
