      ...
      <div className="space-y-5">
        ...
        <div className={glassCardClass('default', 'p-5')}>
          <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${isDark ? 'text-white/50' : 'text-[#160510]/40'}`}>
            {t ? '充值 USD3' : 'Deposit USD3'}
          </div>
          <p className={`text-[11px] mb-4 leading-relaxed ${isDark ? 'text-white/40' : 'text-[#160510]/45'}`}>
            {t ? '使用 USDT 按 1:1 铸造 USD3。USD3 是协议内质押与团队奖励的专用资产，不可直接在 DEX 交易。' : 'Mint USD3 1:1 with USDT. USD3 is used for staking and team rewards; not tradable on DEX.'}
          </p>
          ...
          <div className="ios-glass-inset p-3 mb-5 flex justify-between text-xs">
            ...
            <span className="font-bold" style={{ color: isDark ? '#E0568F' : '#8A2B57' }}>~{depositAmount || '0'} USD3</span>
          </div>
      ...
      <div className="space-y-5">
        ...
        <div className={glassCardClass('accent', 'p-4')}>
          <div className={`text-[11px] leading-relaxed ${isDark ? 'text-[#E0568F]/80' : 'text-[#8A2B57]/80'}`}>
            {t
              ? '推荐奖励为下级入金的 30%，全部以 USD3 入账（用于质押投资，不可提现）。其中 15% 可转让额度仅可转给直推下线。'
              : 'Referral reward is 30% of downline entry, all credited as USD3 for staking/investment — not withdrawable. 15% is transferable to direct downline only.'}
          </div>
        </div>
        <div className={glassCardClass('default', 'p-5')}>
          <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${isDark ? 'text-white/50' : 'text-[#160510]/40'}`}>
            {t ? '转让 USD3 给下线' : 'Transfer USD3 to Downline'}
          </div>
          <div className={`ios-glass-inset p-3 mb-4 flex justify-between text-xs`}>
            ...
            <span className="font-bold text-emerald-500">{transferableLeft} USD3</span>
          </div>
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            />
            <span className={`text-sm ${isDark ? 'text-white/40' : 'text-[#160510]/40'}`}>USD3</span>
          </div>
    ...
    <div className="space-y-5">
      <div className={glassCardClass('highlight', 'p-5 relative overflow-hidden')}>
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            <div className={`text-[10px] ${isDark ? 'text-white/40' : 'text-[#160510]/40'}`}>USD3</div>
      ...
      <div className={glassCardClass('default', 'p-5')}>
        <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${isDark ? 'text-white/50' : 'text-[#160510]/40'}`}>
          {t ? 'USD3 用途' : 'USD3 Usage'}
        </div>
        <ul className={`space-y-2 text-[11px] leading-relaxed ${isDark ? 'text-white/45' : 'text-[#160510]/50'}`}>
          <li>· {t ? 'LP 债券 / 销毁债券入场需质押 USD3（非直接持有 LP 代币）' : 'LP/Burn bond entry requires staking USD3 (not raw LP tokens)'}</li>
          <li>· {t ? '质押 USD3 后按期限获得 D3 释放与 veD3 权重' : 'Stake USD3 to receive D3 vesting and veD3 weight by lock period'}</li>
          <li>· {t ? '推荐奖励 30%（入金）全部以 USD3 到账，用于投资质押，不可提现' : 'Referral 30% of entry paid entirely in USD3 for staking — not withdrawable'}</li>
      ...
      <div className={glassCardClass('default', 'p-5')}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-white/50' : 'text-[#160510]/40'}`}>
            {t ? '推荐奖励 USD3（入金 30%）' : 'Referral rewards USD3 (30% of entry)'}
          </div>
        </div>
