  ...
  return (
    <div className="space-y-4">
      <button
        type="button"
        onClick={onBack}
        className={`flex items-center gap-1.5 text-xs font-medium ios-glass-pressable ${isDark ? 'text-white/50' : 'text-[#2C2824]/50'}`}
      >
      ...
      <div className={glassCardClass('highlight', 'p-5 relative overflow-hidden')}>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h2 className={`text-lg font-bold font-heading ${isDark ? 'text-white' : 'text-[#2C2824]'}`}>
              {t ? project.nameZh : project.name}
            </h2>
            <p className={`text-[10px] mt-0.5 ${isDark ? 'text-white/35' : 'text-[#2C2824]/35'}`}>Gauge · {project.gauge}</p>
          </div>
          <span className={cn(
        ...
        <p className={`text-[11px] leading-relaxed text-pretty-wrap ${isDark ? 'text-white/45' : 'text-[#2C2824]/50'}`}>
        ...
        <div className="space-y-2.5">
          {[
            ...
            <div key={row.label} className="flex items-center justify-between py-1.5 text-xs">
              <span className={isDark ? 'text-white/40' : 'text-[#2C2824]/40'}>{row.label}</span>
              <span className={`font-medium ${isDark ? 'text-white' : 'text-[#2C2824]'}`}>{row.value}</span>
            </div>
          ))}
        </div>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={`mt-4 flex items-center justify-center gap-1.5 text-[11px] font-medium ${isDark ? 'text-[#C9A96E]' : 'text-[#6B1A3A]'}`}
        >
      ...
      <div className={glassCardClass('default', 'p-5')}>
        ...
        <p className={`text-[11px] mb-3 leading-relaxed ${isDark ? 'text-white/40' : 'text-[#2C2824]/45'}`}>
          {t
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={item.label} className="ios-glass-inset p-2.5 text-center">
              <div className={`text-[9px] ${isDark ? 'text-white/30' : 'text-[#2C2824]/30'}`}>{item.label}</div>
              <div className={`text-[11px] font-semibold mt-0.5 ${isDark ? 'text-[#C9A96E]' : 'text-[#6B1A3A]'}`}>{item.value}</div>
              <div className={`text-[8px] ${isDark ? 'text-white/25' : 'text-[#2C2824]/25'}`}>/vote</div>
            </div>
          ))}
        </div>
        <GlassButton
    ...
    <div className="space-y-5">
      <div className={glassCardClass('highlight', 'p-5 relative overflow-hidden')}>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className={`text-xs ${isDark ? 'text-white/40' : 'text-[#2C2824]/40'}`}>{t ? '当期贿赂池' : 'Current Bribe Pool'}</div>
        ...
        <div className="text-3xl font-bold font-heading" style={{ color: isDark ? '#C9A96E' : '#6B1A3A' }}>$2.4M</div>
        <div className={`flex flex-wrap gap-x-4 gap-y-1 mt-3 text-[10px] ${isDark ? 'text-white/40' : 'text-[#2C2824]/40'}`}>
        ...
        {([
          ...
          <button
            ...
            className={cn(
              ...
              filter === f.id ? (isDark ? 'text-white' : 'text-[#6B1A3A]') : (isDark ? 'text-white/40' : 'text-[#2C2824]/40'),
            )}
          >
      ...
      <div className={glassCardClass('default', 'p-4 sm:p-5')}>
        <div className="flex items-center justify-between gap-3 mb-3">
          ...
          <button
            ...
            className={`text-[10px] font-semibold ios-glass-pressable ${isDark ? 'text-[#C9A96E]/80' : 'text-[#6B1A3A]/75'}`}
          >
        ...
        <div className="flex gap-2 mb-3">
          <button
            type="button"
            onClick={() => setRange('7d')}
            className={cn('flex-1 ios-glass-inset py-2 text-[10px] font-semibold ios-glass-pressable', range === '7d' && 'ring-1 ring-[#C9A96E]/20')}
          >
          ...
          <button
            ...
            className={cn('flex-1 ios-glass-inset py-2 text-[10px] font-semibold ios-glass-pressable', range === '30d' && 'ring-1 ring-[#C9A96E]/20')}
          >
            30D
          </button>
          <button
            ...
            className={cn('flex-1 ios-glass-inset py-2 text-[10px] font-semibold ios-glass-pressable', range === 'all' && 'ring-1 ring-[#C9A96E]/20')}
          >
            {t ? '全部' : 'All'}
          </button>
        </div>

        <div className={`text-[10px] mb-2 ${isDark ? 'text-white/30' : 'text-[#2C2824]/35'}`}>
          ...
          {filtered.map((project) => (
            ...
            >
              <div className="flex items-center justify-between mb-1.5">
                <span className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-[#2C2824]'}`}>
                  {t ? project.nameZh : project.name}
                </span>
                <span className="text-xs font-bold" style={{ color: isDark ? '#C9A96E' : '#6B1A3A' }}>{project.bribeAmount}</span>
              </div>
              <div className={`flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] ${isDark ? 'text-white/40' : 'text-[#2C2824]/40'}`}>
