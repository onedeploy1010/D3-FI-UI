/** Polymarket Data API — leaderboard, positions, profile resolve */
...
export type TraderView = {
  ...
  type: string;
  aiAnalysis?: string;
  topPositions?: TraderPositionView[];
...
async function pmGet<T>(url: string): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Polymarket ${res.status}: ${text.slice(0, 200)}`);
  }
  return res.json() as Promise<T>;
}

export async function fetchPmLeaderboard(type: LeaderboardType, limit = 25): Promise<PmLeaderboardRow[]> {
  ...
  if (type === 'rising') {
    u.searchParams.set('timePeriod', 'WEEK');
  } else {
...
export async function fetchPmPositions(
  address: string,
  limit = 20,
  openOnly = false,
...
async function enrichTopPositions(traders: TraderView[], maxTraders = 25): Promise<void> {
  const slice = traders.slice(0, maxTraders);
  const batchSize = 5;
...
export async function resolvePmUsername(username: string): Promise<ResolvePayload> {
  const clean = username.replace(/^@/, '').trim();
  if (!clean) {
    return { address: '', username: clean, profileUrl: '', error: 'Username required' };
  }
  const u = new URL(`${DATA_API}/v1/leaderboard`);
...
function mapTraderPosition(row: PmPositionRow): TraderPositionView {
  return {
    market: row.title,
...
function mapTrader(row: PmLeaderboardRow, index: number, type: LeaderboardType): TraderView {
  ...
  const drawdown = Math.min(
    42,
    Math.round(type === 'rising' ? 10 + volScore * 2 : 14 + volScore * 3),
  );
  ...
  if (tags.length < 3) tags.push('On-chain');
  ...
  return {
    ...
    aiAnalysis: `Rank #${rank} on Polymarket ${type === 'rising' ? 'weekly' : 'all-time'} PnL leaderboard. Volume $${Math.round(vol).toLocaleString('en-US')}, realized PnL $${Math.round(pnl).toLocaleString('en-US')}.`,
...
export function mapPositionsPayload(address: string, rows: PmPositionRow[]): PositionsPayload {
  const username = rows[0]?.proxyWallet ? undefined : undefined;
...
export async function buildPositions(address: string): Promise<PositionsPayload> {
