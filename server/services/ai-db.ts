...
export function walletFromRequest(req: Request): string {
  ...
  return normalizeWallet(fromQuery ?? fromHeader ?? DEMO_WALLET);
}
...
export async function getUserStrategies(userId: string) {
  ...
  const owned = catalog.filter((s) => ownedIds.includes(s.id)).map((s) => ({ ...s, scope: 'user' as const }));
...
export async function purchaseStrategy(userId: string, strategyId: string) {
  ...
  if ((account.tokens ?? 0) < strategy.tokenCost) {
    throw new Error('Insufficient tokens');
  }

  const newTokens = (account.tokens ?? 0) - strategy.tokenCost;
